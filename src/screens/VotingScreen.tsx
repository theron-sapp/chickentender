...
const VotingScreen: React.FC<VotingScreenProps> = ({navigation}) => {
  ...
  //     if (direction === 'right' && session) {
  //       if (!session) {
  //         console.error('No session found');
  //         return;
  //       }
  //       const restaurant = session?.restaurants?.[index];
  //       voteOnRestaurant(session.code, {
  //         userId,
  ...
  //     if (session && index === session.restaurants.length - 1) {
  //       socket.emit('done voting', {sessionCode: session.code, userId});
  ...
  return (
    <View style={styles.container}>
      <Swiper
        ...
        stackSize={2}
        infinite // Set to true if you want to loop the cards after the last one is swiped
